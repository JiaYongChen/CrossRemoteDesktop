...
bool DataValidator::validate(const QByteArray& raw, const QString& mimeType, DataRecord& recordOut) {
    // 基本非空校验
    if ( raw.isEmpty() ) {
        qCWarning(lcServerEncode) << "验证失败：原始数据为空";
        return false;
    }
    if ( mimeType.isEmpty() ) {
        qCWarning(lcServerEncode) << "验证失败：MIME类型为空";
        return false;
    }
    ...
    if ( mimeType.startsWith("image/") ) {
        ...
        if ( img.isNull() ) {
            qCWarning(lcServerEncode) << "验证失败：图像解码失败" << reader.errorString();
            return false;
        }
...
bool DataCleanerFormatter::cleanAndFormat(const DataRecord& in, DataRecord& out, QString& errorOut) {
    ...
    if ( out.mimeType.startsWith("image/") ) {
        ...
        if ( img.isNull() ) {
            ...
            qCWarning(lcServerEncode) << errorOut;
            return false;
        }
        // 统一为ARGB32格式
...
bool InMemoryDataStore::save(const DataRecord& record, QString& errorOut) {
    ...
    qCDebug(lcServerEncode) << "已保存记录" << record.id << "当前总数:" << m_storage.size();
    return true;
}
...
bool InMemoryDataStore::remove(const QString& id, QString& errorOut) {
    ...
    qCDebug(lcServerEncode) << "已删除记录" << id << "当前总数:" << m_storage.size();
    return true;
}
