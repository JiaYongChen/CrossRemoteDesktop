...
**Why:**
1. Ninja 直接调用 cl.exe 时依赖 shell 环境中的 INCLUDE 变量来找 UCRT 标准头文件（stddef.h、limits.h 等）。如果不是从 Developer Command Prompt 启动，INCLUDE 为空，C 语言子项目编译会报 `fatal error C1083: 无法打开包含文件 stddef.h`。
...
- 如果遇到第三方子项目编译报找不到标准头文件，先删除 `build/` 目录重新配置
