...
{
    qCInfo(lcTestClientHandler, "开始 ClientHandlerWorker 基本测试");
}

void TestClientHandlerWorker::cleanupTestCase()
{
    qCInfo(lcTestClientHandler, "ClientHandlerWorker 基本测试完成");
}

void TestClientHandlerWorker::test_basic()
{
    qCInfo(lcTestClientHandler, "执行基本测试");
    
    // 基本测试：验证测试框架工作正常
    QVERIFY(true);
    QCOMPARE(1 + 1, 2);
    
    qCInfo(lcTestClientHandler, "基本测试通过");
}
