...
/// 状态离开 VerifyingTrust 时对话框必须被关闭（不得滞留于健康会话之上），
/// 挂起期拒绝关闭窗口，挂起期接受放行且不关窗
class ConnectionLifecycleTest : public QObject {
    Q_OBJECT
private slots:
    void trustDialogDismissedWhenStateLeavesVerifyingTrust();
    void rejectWhilePendingClosesWindow();
    void acceptWhilePendingProceedsWithoutClosing();
};

void ConnectionLifecycleTest::trustDialogDismissedWhenStateLeavesVerifyingTrust() {
